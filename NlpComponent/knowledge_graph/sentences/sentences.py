...
class Sentences:
    def __init__(self, file_list=None, jieba_dict=abs_path + "nk_dict.txt", update=False, comma=True):
    ...
    def read_homo(self):
        homo_file = abs_path + "homo.pkl"
        if not os.path.exists(homo_file) or self.update:
            with open(abs_path + "homo.txt", "r", encoding='utf-8')as f:
                lines = f.readlines()
                tmp_list = []
                for line in lines:
                    if line:
                        ...
                        self.homo[b] = a
                with open(homo_file, 'wb')as w:
                    pickle.dump(self.homo, w)

        with open(homo_file, "rb")as f:
            self.homo = pickle.load(f)
            if __name__ == '__main__':
                print("近义词数量=", len(self.homo))

    def read_stopwords(self):
        stopword_file = abs_path + "stop_words.pkl"
        if not os.path.exists(stopword_file) or self.update:
    ...
    def toHomo(self, sentence):
        hs = []
        for word in sentence:
            if word in self.homo:
                word = self.homo[word]
            hs.append(word)
        return hs
